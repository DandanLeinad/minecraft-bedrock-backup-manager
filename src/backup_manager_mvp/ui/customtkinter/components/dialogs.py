...
def show_disclaimer_dialog(main_window: ctk.CTk) -> None:
    """Exibe dialog de disclaimer obrigatório na inicialização.

    Args:
        main_window: Janela principal
    """
    # Criar janela de disclaimer
    disclaimer_window = ctk.CTkToplevel(main_window)
    disclaimer_window.title("Aviso Importante")
    disclaimer_window.geometry("600x450")
    disclaimer_window.resizable(False, False)
    disclaimer_window.grab_set()

    # Centralizar janela
    disclaimer_window.transient(main_window)
    ...
    # === BOTÃO DE ACEITAR ===
    button_frame = ctk.CTkFrame(disclaimer_window, fg_color="transparent")
    ...
    disclaimer_window.update()
    main_window.wait_window(disclaimer_window)
